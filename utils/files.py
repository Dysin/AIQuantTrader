...
class FileUtils:
    ...
    def delete(self, search_string='*', type='files'):
        """
        删除指定目录及其子目录中所有文件名包含特定字符串的文件

        参数:
            directory (str): 要搜索的根目录路径
            search_string (str): 文件名中要匹配的字符串
        """
        for file in self.path_utils.glob(search_string):
            if type == 'files':
                ...
                file.unlink()
                shutil.rmtree(file)
        logger.info(f'File removed | {self.path}/{search_string}')
